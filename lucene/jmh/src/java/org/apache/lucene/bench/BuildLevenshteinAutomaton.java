 ...
 */
package org.apache.lucene.bench;
...
// pushd /l/trunk/lucene/core; ../../gradlew jar; popd; javac -cp
// /l/trunk/lucene/core/build/libs/lucene-core-9.0.0-SNAPSHOT.jar BuildLevenshteinAutomaton.java;
// java -cp .:/l/trunk/lucene/core/build/libs/lucene-core-9.0.0-SNAPSHOT.jar
// BuildLevenshteinAutomaton lucene 1 true 2 false 128 > /x/tmp/out.dot; dot -Tpng /x/tmp/out.dot >
// /x/tmp/out.png

// javac -cp /l/buildfst/lucene/build/core/lucene-core-4.4-SNAPSHOT.jar BuildFST.java

/** The type Build levenshtein automaton. */
@SuppressForbidden(reason = "JMH uses std out for user output")
public class BuildLevenshteinAutomaton {

  /** Instantiates a new Build levenshtein automaton. */
  public BuildLevenshteinAutomaton() {}
  ...
  private static boolean parseBoolean(String value, String paramName) {
    if (value.equals("true")) {
      ...
      throw new RuntimeException(
          paramName + " must be \"true\" or \"false\"; got: \"" + value + "\"");
    }
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args.length != 5 && args.length != 6) {
      throw new RuntimeException(
          "Usage: java BuildLevenshteinAutomaton <term> <edit-distance:1|2> <transposition-is-one-edit:true|false> <prefix-length:non-negative-int> <convert-to-utf8:true|false> [<alphabet-max>]");
    }
    ...
    try {
      ...
      throw new RuntimeException(
          "could not parse edit-distance \"" + args[1] + "\" as integer", nfe);
    }
    ...
    try {
      ...
      throw new RuntimeException(
          "could not parse prefix-length \"" + args[1] + "\" as integer", nfe);
    }

    if (prefixLength < 0 || prefixLength > termLength) {
      throw new RuntimeException(
          "prefix-length must be >= 0 and <= term's UTF32 length; got: " + prefixLength);
    }
    ...
    if (args.length == 6) {
      try {
        ...
        throw new RuntimeException(
            "could not parse alphabet-max \"" + args[5] + "\" as integer", nfe);
      }
    } else {
    ...
    Automaton a =
        new LevenshteinAutomata(suffix, alphaMax, transpositionIsOneEdit)
            .toAutomaton(editDistance, prefix);
    if (convertToUTF8) {
