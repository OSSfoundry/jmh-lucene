...
//  // STEPS:
////
////   * Download index data from http://home.apache.org/~ivera/osmdata.wkt.gz. this is a selection
// of polygon data  from OpenStreetMaps corpus and place in a local directory;.
////
////   * Download polygon  files from http://home.apache.org/~mikemccand/geobench and place in a
// local directory.
////
////   * Uncompress files
////
...
////
////   * tweak the commands below to match your classpath, then run with options like "-intersects
// -point -file osmdata.wkt -reindex"
//
// Example how to compile it:
// javac -cp
// path-to-lucene-solr/lucene/build/test-framework/classes/java:path-to-lucene-solr/lucene/build/codecs/classes/java:path-to-lucene-solr/lucene/build/core/classes/java:path-to-lucene-solr/lucene/build/sandbox/classes/java path-to-luceneutil/src/main/perf/IndexAndSearchShapes.java
//
// Example how to run it:
// java -Xmx10g -cp
// path-to-luceneutil/src/main:path-to-lucene-solr/lucene/build/test-framework/classes/java:path-to-lucene-solr/lucene/build/codecs/classes/java:path-to-lucene-solr/lucene/build/core/classes/java:path-to-lucene-solr/lucene/build/sandbox/classes/java perf.IndexAndSearchShapes -intersects -point -file osmdata.wkt

/** The type Index and search shapes. */
...
public class IndexAndSearchShapes {
  ...
  /** Instantiates a new Index and search shapes. */
  public IndexAndSearchShapes() {}
  ...
  private static void createIndex(
    ...
    for (int part = 0; part < NUM_PARTS; part++) {
      org.apache.lucene.store.Directory dir =
          org.apache.lucene.store.FSDirectory.open(Paths.get(getName(part, fileName, isDev)));

      org.apache.lucene.index.IndexWriterConfig iwc =
          new org.apache.lucene.index.IndexWriterConfig(null);
      iwc.setOpenMode(org.apache.lucene.index.IndexWriterConfig.OpenMode.CREATE);
      if (fast) {
        ((org.apache.lucene.index.TieredMergePolicy) iwc.getMergePolicy())
            .setMaxMergedSegmentMB(Double.POSITIVE_INFINITY);
            ...
            for (int latStepEnd = latStep + 1; latStepEnd <= STEPS; latStepEnd++) {
              ...
              for (int lonStepEnd = lonStep + 1; lonStepEnd <= STEPS; lonStepEnd++) {
                ...
                double distanceMeters =
                    org.apache.lucene.util.SloppyMath.haversinMeters(lat, lon, latEnd, lonEnd)
                        / 2.0;
  ...
  private static double[][] makeRegularPoly(
    ...
    for (int i = 0; i < gons; i++) {
      ...
      while (true) {
        ...
        double distanceMeters =
            org.apache.lucene.util.SloppyMath.haversinMeters(centerLat, centerLon, lat, lon);
  ...
  private static String setQueryClass(String currentValue, String newValue) {
    if (currentValue != null) {
      throw new IllegalArgumentException(
          "specify only one of -poly, -polyFile, -distance, -box, -point");
    }
    return newValue;
  }
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    if (queryClass == null) {
      throw new IllegalArgumentException(
          "must specify exactly one of -box, -polyFile or -poly gons; got none");
    }
    if (fileName == null) {
      fileName = "osmdata.wkt";
      // throw new IllegalArgumentException("file name must must be provided using parameter -file
      // <fileName>");
    }
